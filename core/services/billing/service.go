...
func (s *Service) GetOverview(ctx context.Context, userID string) (*models.BillingOverviewResponse, error) {
	...
	plans, err := s.db.GetAllBillingPlans()
	if err != nil {
		return nil, fmt.Errorf("get billing plans: %w", err)
	}

	currentSubscription, err := s.db.GetSubscriptionByUserID(uid)
	...
	invoices, err := s.getInvoiceHistory(ctx, uid)
	if err != nil {
		return nil, fmt.Errorf("get invoice history: %w", err)
	}
	...
	if currentSubscription != nil {
		...
		plan, planErr := s.db.GetBillingPlanByID(currentSubscription.PlanID)
		if planErr != nil {
			return nil, fmt.Errorf("get current billing plan: %w", planErr)
		}
		...
		if response.CurrentPlan.QuotaLimit <= 0 {
			response.CurrentPlan.QuotaLimit = int(response.CurrentPlan.QuotaUsed)
		}
	}

	planSummaries := make([]models.BillingPlanSummary, 0, len(plans))
	for _, plan := range plans {
		planSummaries = append(planSummaries, models.BillingPlanSummary{
			ID:         plan.ID,
			Name:       plan.Name,
			Price:      plan.Price,
			QuotaLimit: plan.MaxMessagesPerDay,
