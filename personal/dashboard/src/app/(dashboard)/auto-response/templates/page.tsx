      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="p-6">
          <div className="mb-5 rounded-xl border border-blue-100 bg-blue-50 p-4 text-sm text-blue-800">
            ...
            Gunakan variabel seperti <code className="rounded bg-blue-100 px-1">{"[Nama]"}</code>, <code className="rounded bg-blue-100 px-1">{"[Phone]"}</code> — template akan otomatis mengganti variabel saat dikirim massal via CSV.
          </div>
                          ...
                          {["[Nama]", "[Phone]", "[Date]", "[Invoice]"].map((v) => (
                            <button
                              ...
                              onClick={() => {
                                ...
                                // Reset cursor position after state update (not perfect in React but helpful)
                                setTimeout(() => {
                                  el.focus();
                                  el.setSelectionRange(start + v.length, start + v.length);
                                }, 0);
                              }}
                              className="rounded bg-white border border-gray-200 px-1.5 py-0.5 text-[10px] font-bold text-gray-500 hover:bg-gray-50 hover:text-green-600"
                            >
                              {v}
                            </button>
                          ))}
