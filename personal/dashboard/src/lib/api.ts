...
let licenseHandler: (() => void) | null = null;

function notifyUnauthorized() {
  unauthorizedHandler?.();
}

function notifyLicenseRequired() {
  licenseHandler?.();
}
...
export function setLicenseHandler(handler: (() => void) | null) {
...
export const configApi = {
  getDbStatus: () =>
    authClient.get<{ success: boolean; status: string; config: any }>("/api/v1/config/db").then((r) => r.data),

  testDbConnection: (body: DbConfig) =>
    authClient.post<{ success: boolean; message: string }>("/api/v1/config/db/test", body).then((r) => r.data),

  saveDbConnection: (body: DbConfig) =>
    authClient.post<{ success: boolean; message: string }>("/api/v1/config/db/save", body).then((r) => r.data),
};
...
export const licenseApi = {
  getStatus: () =>
    authClient.get<{ success: boolean; data: LicenseStatus }>("/api/v1/license/status").then((r) => r.data),

  activate: (key: string) =>
    authClient.post<{ success: boolean; message: string }>("/api/v1/license/activate", { key }).then((r) => r.data),
};
...
export const authApi = {
  ...
  loginWithPassword: (body: { password: string }) =>
    authClient.post<AuthSessionResponse>("/api/v1/auth/login-password", body).then((r) => r.data),
  changePassword: (body: { old_password: string; new_password: string }) =>
    api.post<{ success: boolean; message: string }>("/api/v1/auth/change-password", body).then((r) => r.data),
};
...
export const messagesApi = {
  ...
  bulkSend: (deviceId: string, formData: FormData) =>
    api
      .post<{ count: number; message: string }>(`/api/v1/devices/${deviceId}/messages/bulk`, formData, {
        headers: { "Content-Type": "multipart/form-data" },
      })
      .then((r) => r.data),
  ...
  exportLogs: () =>
    api.get("/api/v1/messages/export", { responseType: "blob" }).then((r) => r.data),
};
...
export const infoApi = {
  ...
  getConfig: () => 
    authClient.get<{ success: boolean; config: Record<string, string> }>("/api/v1/info/config").then((r) => r.data),
};
...
export const integrationApi = {
  getKeys: () => 
    api.get<{ keys: APIKey[] }>("/api/v1/integration/keys").then((r) => r.data),
  
  createKey: (name: string) => 
    api.post<{ key: string; name: string; id: string }>("/api/v1/integration/keys", { name }).then((r) => r.data),
    
  deleteKey: (id: string) => 
    api.delete<{ message: string }>(`/api/v1/integration/keys/${id}`).then((r) => r.data),
    
  getWebhook: () => 
    api.get<WebhookSettings>("/api/v1/integration/webhook").then((r) => r.data),
    
  updateWebhook: (data: Partial<WebhookSettings>) => 
    api.put<{ message: string }>("/api/v1/integration/webhook", data).then((r) => r.data),
};

// --- Settings API (New) ------------------------------------------------------
export const settingsApi = {
  get: () => 
    api.get<{ settings: SystemSetting[] }>("/api/v1/settings").then((r) => r.data),
