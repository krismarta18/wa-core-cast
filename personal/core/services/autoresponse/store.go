...
func (s *Store) CreateKeyword(kw *Keyword) error {
	...
	_, err := s.db.Exec(query, kw.ID, kw.UserID, kw.DeviceID, kw.Keyword, kw.MatchType, kw.ResponseText, kw.IsActive, kw.CreatedAt, kw.UpdatedAt)
	return err
}
...
func (s *Store) GetKeywordsByUserID(userID uuid.UUID) ([]*Keyword, error) {
	...
	for rows.Next() {
		var k Keyword
		if err := rows.Scan(&k.ID, &k.UserID, &k.DeviceID, &k.Keyword, &k.MatchType, &k.ResponseText, &k.IsActive, &k.CreatedAt, &k.UpdatedAt); err != nil {
			return nil, err
		}
...
func (s *Store) UpdateKeyword(kw *Keyword) error {
	query := `
		...
		SET keyword = $1, match_type = $2, response_text = $3, is_active = $4, updated_at = $5
		WHERE id = $6 AND user_id = $7
	`
	kw.UpdatedAt = time.Now()
	_, err := s.db.Exec(query, kw.Keyword, kw.MatchType, kw.ResponseText, kw.IsActive, kw.UpdatedAt, kw.ID, kw.UserID)
	return err
}
...
func (s *Store) GetKeywordByID(id uuid.UUID, userID uuid.UUID) (*Keyword, error) {
	...
	`
	var k Keyword
	err := s.db.QueryRow(query, id, userID).Scan(&k.ID, &k.UserID, &k.DeviceID, &k.Keyword, &k.MatchType, &k.ResponseText, &k.IsActive, &k.CreatedAt, &k.UpdatedAt)
	if err != nil {
		return nil, err
	}
	return &k, nil
