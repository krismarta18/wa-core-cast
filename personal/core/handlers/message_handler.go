...
// Personal Pro: Process Bulk Messages via CSV
// POST /devices/:device_id/messages/bulk
func (h *MessageHandler) ProcessBulkMessages(c *gin.Context) {
	deviceID := c.Param("device_id")

	file, _, err := c.Request.FormFile("file")
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "CSV file is required (form-data key: 'file')"})
		return
	}
	defer file.Close()

	ctx := c.Request.Context()
	count, err := h.messageService.ProcessBulkCSV(ctx, deviceID, file)
	if err != nil {
		utils.Error("Failed to process bulk CSV", zap.String("device_id", deviceID), zap.Error(err))
...
// Personal Pro: Export Message Logs as CSV
// GET /messages/export
func (h *MessageHandler) ExportLogs(c *gin.Context) {
	...
	data, err := h.messageService.ExportLogsCSV(userID)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to generate export"})
		return
	}

	filename := fmt.Sprintf("wacast_logs_%s.csv", time.Now().Format("20060102_1504"))
...
func (h *MessageHandler) GetQueueStats(c *gin.Context) {
	...
	c.JSON(http.StatusOK, QueueStatsResponse{
		...
		TotalFailed:  0, // Not explicitly tracked in simple metrics
		Pending:      0, // Not explicitly tracked in simple metrics
...
func RegisterMessageRoutes(router interface {
...
}, messageService *message.Service, jwtSecret string, authService *auth.Service, integrationService *integration.Service) {
	handler := NewMessageHandler(messageService)

	// Protected group
	group := router.Group("")
	group.Use(IntegrationAuthMiddleware(jwtSecret, authService, integrationService))
	...
	messages := group.Group("/devices/:device_id/messages")
	{
		...
		messages.POST("/bulk", handler.ProcessBulkMessages)       // Bulk send via CSV
	...
	queue := group.Group("/messages")
	{
		...
		queue.GET("/export", handler.ExportLogs)                  // Export logs as CSV
	}
}
