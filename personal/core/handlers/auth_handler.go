...
func RegisterAuthRoutes(v1 *gin.RouterGroup, svc *auth.Service, jwtSecret string) {
	...
	public := v1.Group("/auth")
	{
		...
		public.POST("/login-password", h.LoginWithPassword)
...
func (h *AuthHandler) LoginWithPassword(c *gin.Context) {
	...
	result, err := h.authService.LoginWithPassword(c.Request.Context(), req.Password, c.ClientIP(), c.Request.UserAgent())
	if err != nil {
		switch {
		case errors.Is(err, auth.ErrInvalidPassword):
			c.JSON(http.StatusUnauthorized, errorResponse("INVALID_PASSWORD", err.Error()))
		case errors.Is(err, auth.ErrUserBanned):
			forbidden(c, "USER_BANNED", err.Error())
		default:
			internalError(c, err)
...
func (h *AuthHandler) ChangePassword(c *gin.Context) {
	...
	if err := h.authService.UpdatePassword(c.Request.Context(), userID, req.OldPassword, req.NewPassword); err != nil {
		switch {
		case errors.Is(err, auth.ErrInvalidPassword):
			c.JSON(http.StatusUnauthorized, errorResponse("INVALID_PASSWORD", "Password lama tidak sesuai"))
		default:
			internalError(c, err)
		}
		return
	...
	c.JSON(http.StatusOK, successResponse("Password updated successfully", nil))
}
