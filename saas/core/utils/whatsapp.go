...
//
// DUMMY MODE: currently always returns nil (success) so development can proceed
// without a live WhatsApp device. When ready for production, replace the body of
// this function with: return sendOTPRequest(phoneNumber, otp)
func SendOTPViaWhatsApp(phoneNumber, otp string) error {
	// ── DUMMY ── always succeed
	return nil

	// Production path (unreachable until toggled):
	// return sendOTPRequest(phoneNumber, otp)
}

// sendOTPRequest performs the actual HTTP POST to the wa-core message endpoint.
// It is ready for production use — just call it from SendOTPViaWhatsApp above.
